...
const faq = [
  ...
  { q: 'Como me preparar para a sessão?', a: 'Durma bem, alimente-se antes, hidrate-se e vista roupas confortáveis que deem acesso à área a ser tatuada.' },
]
...
export default function AgendamentoPage() {
  ...
  const WHATSAPP = '5595981019856'

  const handleSubmit = () => {
    if (!form.termos) return

    const msg = [
      '🖤 *NOVO AGENDAMENTO — ESTÚDIO NO NO* 🖤',
      '',
      '*📋 DADOS DO CLIENTE*',
      `Nome: ${form.nome}`,
      `E-mail: ${form.email}`,
      ...
      form.referencia ? `Referência/Inspiração: ${form.referencia}` : '',
      '',
      '*📅 PREFERÊNCIAS*',
      ...
      '',
      form.referencia ? '📸 *O cliente possui foto de referência — peça para enviar aqui no chat!*' : '',
        ...
        {submitted ? (
          <div className="max-w-lg mx-auto text-center py-20">
            ...
            <p className="text-zinc-500 font-body text-sm leading-relaxed mb-3">
              Sua solicitação foi enviada direto para o WhatsApp do estúdio. Aguarde o retorno da nossa equipe para confirmar data, horário e orçamento.
            </p>
            <p className="text-zinc-400 font-body text-xs mb-8">
              Se tiver fotos de referência, envie-as no chat do WhatsApp que acabou de abrir.
            </p>
            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              <a
                href={`https://wa.me/5595981019856`}
                target="_blank"
              ...
              <div className="bg-white border border-zinc-100 rounded-2xl p-4 sm:p-6 md:p-8">
                ...
                {step === 0 && (
                  <div className="space-y-5">
                    <h2 className="font-display text-2xl tracking-wider mb-6">DADOS PESSOAIS</h2>
                    <div>
                      <label className="block text-xs font-body tracking-widest text-zinc-500 uppercase mb-2">Nome Completo *</label>
                      <input
                        type="text"
                        value={form.nome}
                    ...
                    <div>
                      <label className="block text-xs font-body tracking-widest text-zinc-500 uppercase mb-2">Telefone / WhatsApp *</label>
                      <input
                        type="tel"
                        value={form.telefone}
                    ...
                    <div>
                      <label className="block text-xs font-body tracking-widest text-zinc-500 uppercase mb-2">Como nos conheceu?</label>
                      <select
                      ...
                      >
                        ...
                        <option>Passando pela loja</option>
                        <option>Outro</option>
                      </select>
                    </div>
                  </div>
                ...
                {step === 1 && (
                  <div className="space-y-5">
                    <h2 className="font-display text-2xl tracking-wider mb-6">SOBRE A TATTOO</h2>
                    <div>
                      <label className="block text-xs font-body tracking-widest text-zinc-500 uppercase mb-2">Estilo Desejado *</label>
                      <select
                        value={form.estilo}
                      ...
                      >
                        ...
                        {['Realismo', 'Blackwork', 'Tribal', 'Mandala', 'Geométrico', 'Aquarela', 'Fineline', 'Neotradicional', 'Japonês', 'Old School', 'Outro'].map(s => (
                          <option key={s}>{s}</option>
                        ))}
                      </select>
                    </div>
                    <div>
                      <label className="block text-xs font-body tracking-widest text-zinc-500 uppercase mb-2">Descrição da Ideia *</label>
                      <textarea
                    ...
                    <div>
                      <label className="block text-xs font-body tracking-widest text-zinc-500 uppercase mb-2">Orçamento Estimado</label>
                      <select
                    ...
                    <div>
                      <label className="block text-xs font-body tracking-widest text-zinc-500 uppercase mb-2">Referência / Inspiração (opcional)</label>
                      <textarea
                        value={form.referencia}
                        onChange={e => set('referencia', e.target.value)}
                        placeholder="Descreva ou cole o link de uma imagem de referência..."
                        rows={2}
                        className="w-full border border-zinc-200 rounded-lg px-4 py-3 font-body text-sm focus:outline-none focus:border-black transition-colors resize-none"
                      />
                      <div className="flex items-center gap-2 mt-2 text-xs text-zinc-400 font-body">
                        <ImagePlus size={13} />
                        <span>Fotos de referência podem ser enviadas diretamente no WhatsApp após o envio.</span>
                      </div>
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="block text-xs font-body tracking-widest text-zinc-500 uppercase mb-2">Parte do Corpo *</label>
                        <select
                        ...
                        >
                          <option value="">Selecione...</option>
                          {['Braço', 'Antebraço', 'Perna', 'Costas', 'Peito', 'Pescoço', 'Mão', 'Pé', 'Costela', 'Ombro', 'Outro'].map(p => (
                            <option key={p}>{p}</option>
                          ))}
                        </select>
                      </div>
                      <div>
                        <label className="block text-xs font-body tracking-widest text-zinc-500 uppercase mb-2">Tamanho</label>
                        <select
                          value={form.tamanho}
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="block text-xs font-body tracking-widest text-zinc-500 uppercase mb-2">Primeira Tatuagem?</label>
                      ...
                      <div>
                        <label className="block text-xs font-body tracking-widest text-zinc-500 uppercase mb-2">É Cover-up?</label>
                        <div className="flex gap-3">
                          {['sim', 'nao'].map(v => (
                            <button
                              key={v}
                ...
                {step === 2 && (
                  <div className="space-y-5">
                    <h2 className="font-display text-2xl tracking-wider mb-6">PREFERÊNCIAS</h2>
                    <div>
                      <label className="block text-xs font-body tracking-widest text-zinc-500 uppercase mb-2">Dias Disponíveis *</label>
                    ...
                    <div>
                      <label className="block text-xs font-body tracking-widest text-zinc-500 uppercase mb-2">Horário Preferido *</label>
                      <div className="grid grid-cols-2 gap-3">
                        {[
                          { label: 'Manhã', sub: '10h – 13h' },
                ...
                {step === 3 && (
                  <div>
                    <h2 className="font-display text-2xl tracking-wider mb-6">CONFIRMAÇÃO</h2>
                    <div className="space-y-3 mb-8">
                      {[
                        ...
                        { label: 'Parte do Corpo', value: form.parteCopo },
                        { label: 'Tamanho', value: form.tamanho },
                        { label: 'Dias', value: form.dias.join(', ') || '—' },
                        { label: 'Horário', value: form.horario || '—' },
                        { label: 'Orçamento', value: form.orcamento || 'Não informado' },
                        { label: 'Referência', value: form.referencia || 'Não informada' },
                      ].map(item => (
                    ...
                    {form.descricao && (
                      <div className="bg-zinc-50 rounded-xl p-4 mb-6">
                        <p className="text-xs font-body tracking-widest text-zinc-400 uppercase mb-2">Descrição</p>
                    ...
                    <label className="flex items-start gap-3 cursor-pointer">
                      ...
                      <span className="font-body text-sm text-zinc-600 leading-relaxed">
                        Concordo com os{' '}
                        <a href="#" className="text-black underline">Termos de Serviço</a>{' '}
                        e{' '}
                        <a href="#" className="text-black underline">Política de Privacidade</a>{' '}
                        do Estúdio NO NO.
                      </span>
                    </label>
                  </div>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-zinc-50 rounded-2xl p-6 space-y-4">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="font-body text-xs text-zinc-500 leading-relaxed">Respondemos em até 24 horas úteis após o agendamento.</p>
                  </div>
                </div>
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="font-body text-xs text-zinc-500 leading-relaxed">Cancelamentos com menos de 48h de antecedência perdem o sinal (30% do valor).</p>
                  </div>
                </div>
              </div>
              ...
              <div className="border border-zinc-100 rounded-2xl p-6">
                <div className="flex items-center gap-2 mb-5">
                  ...
                  <h3 className="font-display text-lg tracking-widest">DÚVIDAS FREQUENTES</h3>
                </div>
              ...
              <div className="bg-black text-white rounded-2xl p-6 text-center">
                ...
                <p className="font-body text-xs text-zinc-400 mb-1">Prefere conversar diretamente?</p>
                <p className="font-body text-xs text-zinc-500 mb-4">(95) 98101-9856</p>
                <a
                  href="https://wa.me/5595981019856?text=Olá! Gostaria de agendar uma sessão no NO NO."
                  target="_blank"
                  rel="noopener noreferrer"
